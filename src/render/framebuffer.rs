...
impl Framebuffer {
    pub fn new(width: u32, height: u32) -> Self {
        ...
        let bg = Color::BLACK;
        Self { color_buffer: vec![bg; size], width, height, background_color: bg, current_color: Color::WHITE }
    }
    #[inline]
    pub fn clear(&mut self) { self.color_buffer.fill(self.background_color); }
    #[inline]
    pub fn set_pixel(&mut self, x: u32, y: u32) { if x < self.width && y < self.height { self.color_buffer[(y * self.width + x) as usize] = self.current_color; } }
    #[inline]
    pub fn set_pixel_color(&mut self, x: u32, y: u32, color: Color) { if x < self.width && y < self.height { self.color_buffer[(y * self.width + x) as usize] = color; } }
    #[inline]
    pub fn get_pixel(&self, x: u32, y: u32) -> Color { if x < self.width && y < self.height { self.color_buffer[(y * self.width + x) as usize] } else { self.background_color } }
    ...
    pub fn upload_to_texture(&self, tex: &mut Texture2D) { let byte_len = self.color_buffer.len() * std::mem::size_of::<Color>(); let bytes: &[u8] = unsafe { std::slice::from_raw_parts(self.color_buffer.as_ptr() as *const u8, byte_len) }; let _ = tex.update_texture(bytes); }
    pub fn apply_anxiety_blur(&mut self, strength: f32, passes: u32) {
        ...
        for _ in 0..passes.min(3) {
            for y in 1..h-1 {
                let ym = (y-1) as u32; let y0 = y as u32; let yp = (y+1) as u32;
                for x in 1..w-1 {
                    let xm = (x-1) as u32; let x0 = x as u32; let xp = (x+1) as u32;
                    let c  = self.get_pixel(x0,y0);
                    let c1 = self.get_pixel(xm,y0); let c2 = self.get_pixel(xp,y0);
                    let c3 = self.get_pixel(x0,ym); let c4 = self.get_pixel(x0,yp);
    ...
    pub fn apply_vignette(&mut self, intensity: f32) {
        let k = intensity.clamp(0.0, 1.0); if k <= 0.0 { return; }
        let w = self.width as f32; let h = self.height as f32; let cx = w * 0.5; let cy = h * 0.5; let max_r = (cx*cx + cy*cy).sqrt();
        for y in 0..self.height {
            for x in 0..self.width {
                let dx = x as f32 - cx; let dy = y as f32 - cy; let d = (dx*dx + dy*dy).sqrt(); let t = (d / max_r).clamp(0.0, 1.0); let fade = (t.powf(2.0)).min(1.0);
                if fade > 0.2 { let idx = (y * self.width + x) as usize; let c = self.color_buffer[idx]; let dark = 1.0 - k * (fade - 0.2); let mul = |v: u8| -> u8 { (v as f32 * dark).clamp(0.0,255.0) as u8 }; self.color_buffer[idx] = Color::new(mul(c.r), mul(c.g), mul(c.b), c.a); }
            }
        }
    }
